...
public class Problem {
	...
	/**
	 * Production time (including setup time for a given batch size) for a given
	 * class of product
	 */
	...
	public int open(String name) {
		...
		while (encore == 1) {
			...
			if (sg.compareToIgnoreCase("NBR_PRODUCT") == 0)
				np = Integer.parseInt(st.nextToken().trim());

			if ((sg.compareToIgnoreCase("CUSTOMER_HOLDING_COST") == 0)
					|| (sg.compareToIgnoreCase("CUSTOMER_HOLDING_COSTS") == 0)) {
				StringTokenizer st2 = new StringTokenizer(
						st.nextToken().trim(), new String(";"));
				customerHoldingCost = (new Double(st2.nextToken()))
						.doubleValue();
			...
			if ((sg.compareToIgnoreCase("TRANSPORTER_CAPACITY") == 0)) {
				transporter
						.setCapacity(Integer.parseInt(st.nextToken().trim()));
			}

			if (((sg.compareToIgnoreCase("TRANSPORTER_DELIVERY_TIMES") == 0) || (sg
					.compareToIgnoreCase("TRANSPORTER_DELIVERY_TIME") == 0))) {
				transporter.setBatchDeliveryTimes((new Double(st.nextToken()))
						.doubleValue());
			}

			if (((sg.compareToIgnoreCase("TRANSPORTER_DELIVERY_COSTS") == 0) || (sg
					.compareToIgnoreCase("TRANSPORTER_DELIVERY_COST") == 0))) {
				transporter.setBatchDeliveryCosts((new Double(st.nextToken()))
						.doubleValue());
			}

			if (((sg.compareToIgnoreCase("SUPPLIER_PRODUCTION_TIMES") == 0) || (sg
					.compareToIgnoreCase("SUPPLIER_PRODUCTION_TIME") == 0))) {
				StringTokenizer st2 = new StringTokenizer(
						st.nextToken().trim(), new String(";"));
				productionTime = (new Double(st2.nextToken())).doubleValue();
			}

			if (((sg.compareToIgnoreCase("SUPPLIER_SETUP_TIMES") == 0) || (sg
					.compareToIgnoreCase("SUPPLIER_SETUP_TIME") == 0))) {
				StringTokenizer st2 = new StringTokenizer(
						st.nextToken().trim(), new String(";"));
				setupTime = (new Double(st2.nextToken())).doubleValue();
			}

			if (((sg.compareToIgnoreCase("SUPPLIER_SETUP_COSTS") == 0) || (sg
					.compareToIgnoreCase("SUPPLIER_SETUP_COST") == 0))) {
				StringTokenizer st2 = new StringTokenizer(
						st.nextToken().trim(), new String(";"));
				setupCost = (new Double(st2.nextToken())).doubleValue();
			}

			if (((sg.compareToIgnoreCase("SUPPLIER_HOLDING_COSTS") == 0) || (sg
					.compareToIgnoreCase("SUPPLIER_HOLDING_COST") == 0))) {
				StringTokenizer st2 = new StringTokenizer(
						st.nextToken().trim(), new String(";"));
				supplierUnitHoldingCost = (new Double(st2.nextToken()))
						.doubleValue();
			}

			if (((sg.compareToIgnoreCase("SUPPLIER_PRODUCTION_HOLDING_COSTS") == 0) || (sg
					.compareToIgnoreCase("SUPPLIER_PRODUCTION_HOLDING_COST") == 0))) {
				StringTokenizer st2 = new StringTokenizer(
						st.nextToken().trim(), new String(";"));
